...
export interface IHabit {
    title: string;
    startTime: string; // habit start in schedule
    endTime: string; // habit end in schedule
    points: number; // points to quantify the progress
    repeat: WeekDays[];
    id: string;
    createdAt: Date;
    lastCheck: number; // the last check day
    icon?: string;
    description?: string;
}
